...
class BBCodeTest extends K.BBCode
  ...
  _parseBBCommand: (state, command, prefix) ->
    if prefix != ''
      if @testValue.prefixedCnt[prefix]?
        @testValue.prefixedCnt[prefix]++
      else
        @testValue.prefixedCnt[prefix] = 1

    switch (command)
      when 'open'
        args = []
        theTag = {command:command, args:args}
        @testValue.tags.push(theTag)
        @_parseBBCommandArgs(state, command, prefix, args)
      else
        super(state, command, prefix)
        @testValue.defaultBBCalledCnt++
